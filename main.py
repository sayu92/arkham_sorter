...
def update(source : str, ratio : float = 1.4225, reduc : float = 0.8, processing_resolution  : tuple = (569, 400), resize : bool = True):
    ...
    roi_boxname1_centroid = [posx_namebox1 + roi_pt1[0], posy_namebox1 + roi_pt1[1]]
    roi_boxname1_pt1 = [roi_boxname1_centroid[0] - round(size_ratio_boxname1[0]*frame.shape[1])//2, roi_boxname1_centroid[1] - round(size_ratio_boxname1[1]*frame.shape[0])//2]
    roi_boxname1_pt2 = [roi_boxname1_centroid[0] + round(size_ratio_boxname1[0]*frame.shape[1])//2, roi_boxname1_centroid[1] +round(size_ratio_boxname1[1]*frame.shape[0])//2]
    roi_boxname1 = frame[roi_boxname1_pt1[1]: roi_boxname1_pt2[1], roi_boxname1_pt1[0]:roi_boxname1_pt2[0]]
    ...
    cv.rectangle(frame,roi_boxname1_pt1, roi_boxname1_pt2, thickness=2, color=(0,255,0) )
    ...
    roi_boxname2_centroid = [posx_namebox2 + roi_pt1[0], posy_namebox2 + roi_pt1[1]]
    roi_boxname2_pt1 = [roi_boxname2_centroid[0] - round(size_ratio_boxname2[0]*frame.shape[1])//2, roi_boxname2_centroid[1] - round(size_ratio_boxname2[1]*frame.shape[0])//2]
    roi_boxname2_pt2 = [roi_boxname2_centroid[0] + round(size_ratio_boxname2[0]*frame.shape[1])//2, roi_boxname2_centroid[1] +round(size_ratio_boxname2[1]*frame.shape[0])//2]
    roi_boxname2 = frame[roi_boxname2_pt1[1]: roi_boxname2_pt2[1], roi_boxname2_pt1[0]:roi_boxname2_pt2[0]]
    ...
    #cv.rectangle(frame,roi_boxname2_pt1, roi_boxname2_pt2, thickness=2, color=(0,255,0) )
    return frame, roi_boxname1, roi_boxname2

if __name__ == '__main__':
    
    while True :
        roi_capture, boxname1, boxname2 = update(path_ip, reduc=0.8, resize=True)
        ...
        for boxname in [boxname1, boxname2]: 
            ...
            if len(result) == 1: # Any other answer is a false positive
                ...
                cv.putText(roi_capture, f"Page: {result[0]}, Position: {result[1]}", (300,300))
            
            cv.imshow("camera", roi_capture)
